$(document).ready(function() {
	...
	function dealCmd(command, term) {
		if (command.indexOf("ls") > -1) {
			$.ajax({
				type : 'get',
				dataType : 'json',
				url : '/AnotherKafkaMonitor/cluster/zk/cmd/ajax?cmd=' + command + '&type=ls',
			...
			$.ajax({
				type : 'get',
				dataType : 'json',
				url : '/AnotherKafkaMonitor/cluster/zk/cmd/ajax?cmd=' + command + '&type=delete',
				success : function(datas) {
					if (datas != null) {
						term.echo(new String(datas.result));
					}
			...
			$.ajax({
				type : 'get',
				dataType : 'json',
				url : '/AnotherKafkaMonitor/cluster/zk/cmd/ajax?cmd=' + command + '&type=get',
				success : function(datas) {
					if (datas != null) {
						term.echo(new String(datas.result));
					}
	...
	$.ajax({
		type : 'get',
		dataType : 'json',
		url : '/AnotherKafkaMonitor/cluster/zk/islive/ajax',
		success : function(datas) {
			if (datas != null) {
				if (datas.live) {
