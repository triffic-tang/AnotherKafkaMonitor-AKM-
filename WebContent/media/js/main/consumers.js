$(document).ready(function() {
	...
	d3.json('/AnotherKafkaMonitor/consumers/info/ajax', function(json) {
		...
		$("#result").dataTable({
			...
			"sAjaxSource" : "/AnotherKafkaMonitor/consumer/list/table/ajax",
			"aoColumns" : [ {
				"mData" : 'id'
			}, {
				"mData" : 'group'
	...
	$(document).on('click', 'a[class=link]', function() {
		...
		$("#result_children" + offset).dataTable({
			...
			"bServerSide" : true,
			"fnServerData" : retrieveData,
			"sAjaxSource" : "/AnotherKafkaMonitor/consumer/" + group + "/table/ajax",
			"aoColumns" : [ {
				"mData" : 'id'
			}, {
				"mData" : 'topic'
