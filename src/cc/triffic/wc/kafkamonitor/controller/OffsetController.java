...
public class OffsetController {
	...
	public void offsetDetailAjax(@PathVariable("group") String group, @PathVariable("topic") String topic, HttpServletResponse response, HttpServletRequest request) {
		...
		for (Iterator<?> offsetIterator = jsonArray.iterator(); offsetIterator.hasNext();) {
			Object obj = offsetIterator.next();
		...
		for (Iterator<?> logIterator = ret.iterator(); logIterator.hasNext();) {
			Object tmp = logIterator.next();
			...
			if ((offset < iDisplayLength + iDisplayStart) && (offset >= iDisplayStart)) {
				...
				if (tmp2.getLong("offset").longValue() == -1L) {
					...
					obj.put("offset", new StringBuilder()
									.append("<a class='btn btn-success btn-xs'>")
									.append(tmp2.getLong("offset"))
									.append("</a>").toString());
				}
				obj.put("lag", new StringBuilder()
								.append("<a class='btn btn-danger btn-xs'>")
								.append(tmp2.getLong("lag")).append("</a>").toString());
